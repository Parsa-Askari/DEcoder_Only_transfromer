...
class Decoder(nn.Module):
    ...
    def forward(self,x):
        ...
        for k in reversed(self.key_cache[:-1]):
            softmax_in=torch.concat((torch.sum(k*query,dim=1),softmax_in),dim=1) #each have the shape (batch_Size,1)
        # softmax_in :(batch_size,seq_length)
        # caclulate the softmax        
        
        attentions=torch.unsqueeze(self.softmax(softmax_in),dim=-1)  # (batch_size,seq_length,1)
